...
function uniqueId() {
  return crypto?.randomUUID ? crypto.randomUUID() : performance.now().toString();
}
 ...
 */
function stringToMessageResponseFormat(string: string): MessageResponse {
  const textItem: GenericItem = {
    response_type: 'text',
    text: string,
  };
  return {
...
export { stringToMessageResponseFormat };
