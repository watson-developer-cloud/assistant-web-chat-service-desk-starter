 ...
 class kustomerDesk implements ServiceDesk {
   ...
   async startChat(connectMessage: MessageResponse): Promise<void> {
    ...
    KustomerCore.init({
      ...
      /*KustomerCore.login({
        jwtToken: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Im1hcnNoYUBleGFtcGxlLmNvbSIsImlhdCI6MTYxODY3Njc5OX0.BYW5ar2Ilgl2XMd01gxjWwUJ-_tl5Okl_P_d0Rf7uv0'
   ...
   private onMessageReceivedHandler = (response, error) => {
   ...
   private onMessageSent = (response, error) => {
     console.log('onMessageSent, this is your agent response', response);
   }

   private onAgentTypingActivity = (response, error) => {
   ...
   private onConversationCreate = (response, error) => {
     if(!this.isChatInit) {
   ...
   private onConversationEndedHandler = (response, error) => {
